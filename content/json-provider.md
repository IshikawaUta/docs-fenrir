...
> **Note:** When orjson is installed (default in v4.4.0), `DefaultJSONProvider` uses `orjson.dumps` which natively handles `datetime`, `date`, `UUID`, and more. Falls back to stdlib `json.dumps` with a custom `default` handler when orjson is not available.
